...
- **씬에 프리팹 없음** — 코드로 동적 생성, 원 스프라이트도 코드로 그림
...
## 7. 코드의 묘미 (Why It Works)

1. **데이터 주도 디자인** — 색/크기 배열만 바꾸면 11종 과일이 자동으로 다르게 보임
2. **InstanceID 비교로 중복 머지 방지** — `merged` 플래그와 함께 2중 안전장치
3. **절차적 스프라이트** — 에셋 의존성 0, 빌드 크기 최소
4. **싱글톤 + 위임** — Fruit는 가벼운 데이터/충돌 핸들러, 모든 상태는 SuikaGame
5. **드롭 후 1.5초 + 정지 조건** — 게임오버 오판 방지의 핵심 트릭
